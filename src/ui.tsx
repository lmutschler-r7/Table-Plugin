            ...
            {options.map((opt, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* right: sort icon button (hover @ 20%, icon-hover @ 50%, stays visible when active) */}
                  <button
                    type="button"
                    onClick={(e) => {
                      e.stopPropagation()
                    ...
                    onMouseEnter={(e) => {
                      ;(e.currentTarget as HTMLButtonElement).style.opacity = active ? '1' : '0.5'
                    }}
                    onMouseLeave={(e) => {
                      ;(e.currentTarget as HTMLButtonElement).style.opacity = active ? '1' : (showHover ? '0.2' : '0')
                    }}
                    style={{
                      border: 'none',
...
function Plugin() {
  ...
  useEffect(() => {
    ...
    if (source.kind === 'builtin') {
      ...
      // custom is handled by onCustomPick -> handleFile
    }
  ...
  // Fit window height to content + rAF throttle for resize messages
  ...
  useEffect(() => {
    resizeToFit()
  }, [fileName, headers.length, source && ('kind' in source ? (source.kind === 'builtin' ? source.id : source.filename) : '')])
  ...
  function onGenerate() {
    ...
    emit('CSV_PARSED', { headers: selectedHeaders, rows: filteredRows, sort })
  }
